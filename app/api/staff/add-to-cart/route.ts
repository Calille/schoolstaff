...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if request already exists in cart (pending, no batch_id)
    ...
    if (existingRequest) {
      return NextResponse.json(
        { error: 'This staff member is already in your request cart' },
        { status: 400 }
      )
    }

    // Create the request (without batch_id - goes to cart)
    const { data, error } = await supabase
      ...
      .insert({
        ...
        batch_id: null, // Explicitly set to null for cart items
      })
      .select()
      .single()
